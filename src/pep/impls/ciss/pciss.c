...
static PetscErrorCode PEPCISSResizeBases(BV S,BV V,BV pV,BV Y,PetscInt Lold,PetscInt Lnew,PetscInt M,PetscInt npoints)
{
  PetscErrorCode   ierr;
  PetscInt         i,j;

  PetscFunctionBegin;
  ierr = BVResize(S,Lnew*M,PETSC_TRUE);CHKERRQ(ierr);
  ierr = BVResize(V,Lnew,PETSC_TRUE);CHKERRQ(ierr);
  if (pV) { ierr = BVResize(pV,Lnew,PETSC_TRUE);CHKERRQ(ierr); }
...
PetscErrorCode PEPSolve_CISS(PEP pep)
{
  ...
  if (L_add>0) {
    ...
    ierr = PEPCISSResizeBases(ctx->S,ctx->V,contour->pA?ctx->pV:NULL,ctx->Y,ctx->L,ctx->L+L_add,ctx->M,contour->npoints);CHKERRQ(ierr);
